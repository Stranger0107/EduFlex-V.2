...
export default function ProfessorMeetings() {
  const { fetchProfessorMeetings } = useApp();
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      const data = await fetchProfessorMeetings();
      setMeetings(data || []);
      setLoading(false);
    };
    load();
  ...
  const loadMessages = async (meetingId) => {
    setMsgLoading(true);
    const res = await fetch(`/api/professor/meetings/${meetingId}/messages`, { headers: { 'Authorization': `Bearer ${localStorage.getItem('authToken')}` } });
  ...
  const sendMessage = async (meetingId) => {
    ...
    try {
      const res = await fetch(`/api/professor/meetings/${meetingId}/messages`, {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="p-8 min-h-screen">
      <h2 className="text-2xl font-bold mb-4">My 1:1 Meetings</h2>
      {loading ? (
        <div>Loading...</div>
      ) : meetings.length === 0 ? (
            ...
            <div key={m._id} className="p-4 border rounded bg-white shadow-sm">
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <div className="text-sm text-gray-600">With: {m.student?.name || 'Student'}</div>
              ...
              {chatOpen === m._id && (
                <div className="mt-4 border-t pt-4">
                  <h4 className="font-semibold mb-2">Chat with {m.student?.name || 'Student'}</h4>
                  {msgLoading ? <div>Loading messages...</div> : (
                    <div className="max-h-64 overflow-y-auto mb-2 bg-gray-50 p-2 rounded">
                      {messages.length === 0 ? <div className="text-gray-400">No messages yet.</div> : messages.map(msg => (
                        <div key={msg._id} className={`mb-2 flex ${msg.sender?._id === m.student?._id ? 'justify-start' : 'justify-end'}`}>
                          <div className={`px-3 py-2 rounded-lg ${msg.sender?._id === m.student?._id ? 'bg-gray-200 text-gray-800' : 'bg-green-200 text-green-900'}`}>
